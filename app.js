...
var TELEMETRY_RATE = (argv.rate ? argv.rate : (process.env.TELEMETRY_RATE || 1));
...
Map.prototype.createMapFromJSON = function(json) {
	...
	var minlat = parseFloat(json.osm.bounds[0].$.minlat);
	var minlon = parseFloat(json.osm.bounds[0].$.minlon);
	var maxlat = parseFloat(json.osm.bounds[0].$.maxlat);
	var maxlon = parseFloat(json.osm.bounds[0].$.maxlon);
	//console.log(JSON.stringify(json));
	for (var i in json.osm.node) {
		var n = json.osm.node[i];
		var xlon = parseFloat(n.$.lon);
		var xlat = parseFloat(n.$.lat);
		xlon = Math.max(Math.min(xlon,maxlon),minlon);
		xlat = Math.max(Math.min(xlat,maxlat),minlat);

		this.nodes[n.$.id] = {
			id: n.$.id,
			lon: xlon, //parseFloat(n.$.lon),
			lat: xlat, //parseFloat(n.$.lat),
			edges: Array()
		}
		this.nodeCount++;
		if (this.nodeCount % 100 == 0) { console.log("nodes: " + this.nodeCount); }
	}
...
//var loaded_map = new Map("austin", "New Delhi", "maps/new-delhi-map.osm");
var loaded_map = new Map("kigoma", "Kigoma - Tz", "maps/kigoma-zoom.osm");
